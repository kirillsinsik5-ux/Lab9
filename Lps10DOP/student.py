...
class Student:
    ...
    def add_multiple_grades(self, grades_list):
        ...
        Args:
            grades_list (list): Список словарей вида [{"grade": 5, "subject": "Math"}, ...]
        ...
        for item in grades_list:
            success = self.add_grade(item.get("grade"), item.get("subject", "General"))
            if success:
                count += 1
        print(f"📋 Добавлено оценок: {count} из {len(grades_list)}")
        return count
