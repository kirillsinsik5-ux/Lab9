...
from mypackage.utils.formatters import format_group_report
...
def demo_package_usage():
    ...
    students_data = [
        ...
        ("Петр",    "Иванов",    18, "petr.ivanov"),           # Некорректный email
    ...
    for first, last, age, email in students_data:
        ...
        if not valid:
            print(f"⚠️  {msg} для {email}")
            email = None

        valid, msg = validate_age(age)
        if not valid:
            print(f"⚠️  {msg} для {first} {last}")
            age = 18
...
def interactive_mode():
    ...
    while True:
        ...
        print("5. Показать ещё одну случайную цитату")
        print("0. Выход")
        ...
        if choice == "0":
            ...
            print_colored("📖 Запрашиваем новую цитату...", "blue")
            quote = client.get_random_quote()
            if quote:
                print_colored(f"\n\"{quote['text']}\"", "yellow")
