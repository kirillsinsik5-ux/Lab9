...
def validate_student_name(first_name, last_name):
    ...
    Returns:
        tuple: (is_valid, error_message)
    """
    for field, value in [("Имя", first_name), ("Фамилия", last_name)]:
        if not value or not isinstance(value, str):
            return False, f"{field} не может быть пустым"
        if not value[0].isupper():
            return False, f"{field} должно начинаться с заглавной буквы"
        if not value.replace('-', '').isalpha():
            return False, f"{field} должно содержать только буквы (допустим дефис)"
